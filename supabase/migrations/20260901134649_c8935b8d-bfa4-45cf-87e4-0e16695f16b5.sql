...
BEGIN
  ...
  IF NOT public.user_owns_company(v_delivery.company_id)
     AND NOT public.has_role(auth.uid(), 'admin'::public.app_role) THEN
    RETURN jsonb_build_object('success', false, 'error', 'FORBIDDEN');
  END IF;

  IF v_delivery.status IN ('completed'::public.delivery_status, 'cancelled'::public.delivery_status) THEN
...
REVOKE ALL ON FUNCTION public.update_delivery_with_credits(uuid, jsonb) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.update_delivery_with_credits(uuid, jsonb) TO authenticated, service_role;